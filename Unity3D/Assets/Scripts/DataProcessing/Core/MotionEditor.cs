...
public class MotionEditor : MonoBehaviour {
	...
	public MotionEditor Char2Editor = null;
	...
	//todo
	public void SetChar2(MotionEditor Char2editor) {
		Char2Editor = Char2editor;
	}
	...
	public Actor GetActor() {
		...
		if(Actor == null) {
			...
			// t.position = new Vector3(-1*t.position.x, t.position.y, -1*t.position.z);
		...
		public void Inspector() {
							...
							using(new EditorGUILayout.VerticalScope ("Box")) {
								...
								if(Target.Playing) {
									if(Utility.GUIButton("||", Color.red, Color.black, 50f, 40f)) {
										...
										Target.Char2Editor.StopAnimation();
									}
								} else {
									if(Utility.GUIButton("|>", Color.green, Color.black, 50f, 40f)) {
										Target.PlayAnimation();
										Target.Char2Editor.Timestamp = Target.Timestamp;
										Target.Char2Editor.PlayAnimation();
									}
								}
			...
			using(new EditorGUILayout.VerticalScope ("Box")) {
				...
				using(new EditorGUILayout.VerticalScope ("Box")) {
					...
					Target.SetChar2((MotionEditor)EditorGUILayout.ObjectField("Character2Editor", Target.Char2Editor, typeof(MotionEditor), true)); //todo
