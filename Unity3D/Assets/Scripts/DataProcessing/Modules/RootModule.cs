...
public class RootModule : Module {
	...
	public bool ReverseRootSeriesDiretion = false;
	...
	protected override void DerivedInspector(MotionEditor editor) {
		...
		ReverseRootSeriesDiretion = EditorGUILayout.Toggle("ReverseRootSeriesDiretion", ReverseRootSeriesDiretion);
	...
	private Vector3 RootPosition(float timestamp, bool mirrored) {
		...
		Vector3 RootPosition(Frame frame) {
			...
			//todo
			return position;
			// return new Vector3(-1*position.x, position.y, -1*position.z);
		}
	}
	...
	private Quaternion RootRotation(float timestamp, bool mirrored) {
		...
		Quaternion RootRotation(Frame frame) {
			if(Topology == TOPOLOGY.Biped) {
				...
				Vector3 forward;
				if(!ReverseRootSeriesDiretion) {forward = Vector3.ProjectOnPlane(-Vector3.Cross(v, Vector3.up), Vector3.up).normalized;}
				else {forward = Vector3.ProjectOnPlane(Vector3.Cross(v, Vector3.up), Vector3.up).normalized;}

				// Vector3 forward1 = Vector3.ProjectOnPlane(-Vector3.Cross(v, Vector3.up), Vector3.up).normalized;
				// Vector3 forward2 = Vector3.ProjectOnPlane(Vector3.Cross(v, Vector3.up), Vector3.up).normalized;
				// Vector3 hip_forward = Vector3.ProjectOnPlane(frame.GetBoneTransformation(Hips, mirrored).GetForward(), Vector3.up).normalized;
				// Vector3 forward = (Vector3.Angle(forward1, hip_forward) < Vector3.Angle(forward2, hip_forward)) ? forward1 : forward2; //? mixamo 模型导入后root trajectory 会反方向, 所以让root forward跟hip forward一致

				Quaternion rotation = (forward == Vector3.zero ? Quaternion.identity : Quaternion.LookRotation(forward, Vector3.up));
				return rotation;
			}
