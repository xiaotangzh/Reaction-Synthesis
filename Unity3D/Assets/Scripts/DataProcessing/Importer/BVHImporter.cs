...
public class BVHImporter : EditorWindow {
		...
		using(new EditorGUILayout.VerticalScope ("Box")) {
			...
			using(new EditorGUILayout.VerticalScope ("Box")) {
				...
				using(new EditorGUILayout.VerticalScope ("Box")) {
					...
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("Assets/MyScene/", GUILayout.Width(100));
	...
	private IEnumerator ImportMotionData() {
		string destination = "Assets/MyScene/" + Destination;
			...
			for(int f=0; f<Assets.Length; f++) {
				if(Assets[f].Import) {
					...
					if(!Directory.Exists(destination+"/"+assetName) ) {
						...
						for(int k=0; k<data.GetTotalFrames(); k++) { // for all frames
							Matrix4x4[] matrices = new Matrix4x4[data.Source.Bones.Length]; //!
							int idx = 0;
							for(int i=0; i<data.Source.Bones.Length; i++) { // for all bones
								...
								// String GrandParent = data.Source.FindBone(info.Parent).Parent;
								// if(GrandParent!="None") {
								// 	Vector3 aligned = matrices[data.Source.FindBone(GrandParent).Index].GetPosition() - matrices[data.Source.FindBone(info.Parent).Index].GetPosition();
								// 	ParentRotation = matrices[data.Source.FindBone(info.Parent).Index]
								// 	rotation = Quaternion.FromToRotation(matrices[data.Source.FindBone(info.Parent).Index].GetRight(), aligned) * matrices[data.Source.FindBone(info.Parent).Index].GetRotation();
								// }

								
								position = (position == Vector3.zero ? offsets[i] : position) * Scale;
								Matrix4x4 local = Matrix4x4.TRS(position, rotation, Vector3.one); //! local coordinate
							...
							// 	MotionData.Hierarchy.Bone info = data.Source.Bones[i];
							// 	Vector3 childPosition = matrices[i].GetPosition();
							// 	Quaternion childRotation = matrices[i].GetRotation();
							// 	String GrandParent = data.Source.FindBone(info.Parent).Parent;
							// 	Debug.Log(info.Name+"  "+GrandParent);
							// 	if(GrandParent!="None") {
							// 		Vector3 ParentPosition = matrices[data.Source.FindBone(info.Parent).Index].GetPosition();
							// 		Vector3 GrandParentPosition = matrices[data.Source.FindBone(GrandParent).Index].GetPosition();
							// 		Quaternion ParentRotation = matrices[data.Source.FindBone(info.Parent).Index].GetRotation();
							// 		Vector3 aligned = GrandParentPosition - ParentPosition;
							// 		Quaternion newParentRotation = Quaternion.FromToRotation(ParentRotation.GetRight(), aligned) * ParentRotation;
							// 		Matrix4x4 newMatrix = Matrix4x4.TRS(ParentPosition, newParentRotation, Vector3.one);
							// 		matrices[data.Source.FindBone(info.Parent).Index] = newMatrix;
							// 	}
							// }
