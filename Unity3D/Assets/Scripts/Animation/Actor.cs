...
public class Actor : MonoBehaviour {
	...
	public bool DrawJointTrajectory = false;
	public Actor DrawJointDistance;
	...
	public void Draw() {
		...
		// if(DrawJointTrajectory) {
		// 	List<int> indices = new List<int> {0, 2, 4, 21};
		// 	// List<int> indices = new List<int> {18, 21, 25}; // 18: Lhand, 21: Head, 25: Rhand
		// 	Color c;
		// 	if(transform.GetComponentInChildren<Char1Controller>() != null) {
		// 		c = new Color(0f, 0.2838f, 0.5283f, 0.5f);
		// 	}
		// 	else {
		// 		c = new Color(0.5960f, 0f, 0f, 0.5f);
		// 	}
		// 	int step = Mathf.Max(Sampling, 1);
		// 	for(int i=step+3; i<History.Count; i+=step) {
		// 		foreach(int bone in indices) {
		// 			UltiDraw.Begin();
		// 			UltiDraw.DrawLine(History[i-step].Transformations[bone].GetPosition(), History[i].Transformations[bone].GetPosition(), 0.02f, c);
		// 			UltiDraw.End();
		// 		}
		// 	}
		// }
		
		if(DrawJointDistance!=null && Application.isPlaying) {
			// List<string> DrawJoints = new List<string> {"Head", "LeftHand", "RightHand", "LeftFoot", "RightFoot", "LeftUpLeg", "RightUpLeg"};
			List<string> DrawJoints = new List<string> {"Head", "Neck", "Hip", "Spine2","LeftHand", "RightHand", "LeftArm", "RightArm", "LeftShoulder", "RightShoulder", "LeftFoot", "RightFoot", "LeftUpLeg", "RightUpLeg", "LeftLeg", "RightLeg"}; // all joint
			foreach(string joint1 in DrawJoints) {
				foreach(string joint2 in DrawJoints) {
					Vector3 pos = FindBoneContains(joint1).Transform.position;
					// Vector3 target_pos = DrawJointLines.FindBoneContains(joint2).Transform.position; // pairwise joint
					Vector3 target_pos = DrawJointDistance.FindBoneContains(joint1).Transform.position; // parallel joint
					Color startColor = transform.GetComponentsInChildren<MeshRenderer>()[0].material.color;
					Color endColor = DrawJointDistance.transform.GetComponentsInChildren<MeshRenderer>()[0].material.color;
					startColor.a = 0.7f;
					endColor.a = 0.7f;

					// Color colorGreen = new Color(0.439f, 0.678f, 0.278f, 0.7f);
					// startColor = colorGreen;
					// endColor = colorGreen;

					// Color targetColor = DrawJointLines.transform.GetComponentsInChildren<MeshRenderer>()[0].material.color;
					// startColor = targetColor;
					// endColor = targetColor;

					UltiDraw.Begin();
					UltiDraw.DrawLine(pos, target_pos, 0.012f, startColor, endColor);
					UltiDraw.End();
				}
			}
		}
	...
	public class Actor_Editor : Editor {
		...
		public override void OnInspectorGUI() {
			...
			Target.DrawJointTrajectory = EditorGUILayout.Toggle("Draw Joint Trajectory", Target.DrawJointTrajectory);
			...
			Target.DrawJointDistance = (Actor)EditorGUILayout.ObjectField("Draw joint lines", Target.DrawJointDistance, typeof(Actor), true);
